...
from Multiwing_multipoint_OAS_indynamics import OASAeroMulti

############### eVTOL input dictionary ####################
...
class eVTOL2DODE(om.Group):
    ...
    def initialize(self):
        ...
        self.options.declare('OAS_surface_1', types=dict, default={}, desc='Surface dict for OAS for the wing') 
        self.options.declare('OAS_surface_2', types=dict, default={}, desc='Surface dict for OAS for the tail')   # not needed if aero_model == "simple"
    ...
    def setup(self):
        ...
        # self.add_subsystem(name='AOA',subsys=AOAComp, promotes_inputs=['vx','vy','theta'],promotes_outputs=['v','alpha'])
        # self.add_subsystem(name='aero', subsys=AeroMulti(num_nodes=nn, OAS_surface_1=self.options['OAS_surface_1'], OAS_surface_2 = self.options['OAS_surface_2']), promotes_inputs=['v', 'alpha','rho'])#, ('S', 'Sref')])
        
        self.add_subsystem(name='AOA',subsys=AOAComp(num=nn), promotes_inputs=['vx','vy','theta'],promotes_outputs=[('v_inf','v'),('aoa','alpha')])
        self.add_subsystem(name='aero', subsys=OASAeroMulti(num_nodes=nn, OAS_surface_1=self.options['OAS_surface_1'], OAS_surface_2 =self.options['OAS_surface_2']), promotes_inputs=['v', 'alpha','rho'])
        
        #---Flight Dynamics ---#
        self.add_subsystem(name='fd', subsys=Dynamics(num_nodes=nn,input_dict=input_dict), promotes_inputs=['vx','vy','theta','power'])
        ...
        # for i in range(nn):
        #     self.connect('aero.aero_point_{}.CD'.format(i), 'fd.Cd_{}'.format(i))
        #     self.connect('aero.aero_point_{}.CL'.format(i), 'fd.Cl_{}'.format(i))



class AOAComp_single(ExplicitComponent):
    def setup(self):
...
## ---- AOA component for multipoint model -- #
...
class Dynamics(ExplicitComponent):
    ...
    def setup(self):
        ...
        #     self.add_input('Cl_{}'.format(j), val=1.0)
        #     self.add_input('Cd_{}'.format(j), val=0.1)
        self.add_input('CL', val=np.ones(num_nodes))
        self.add_input('CD', val=0.1*np.ones(num_nodes))
    ...
    def compute(self, inputs, outputs):
        ...
        for i in range(self.options['num_nodes']):
            ...
            # outputs['CL'][i] = step[2]
            # outputs['CD'][i] = step[3]
            # outputs['CL'][i] = inputs['Cl_'.format(i)] 
            # outputs['CD'][i] = inputs['Cd_'.format(i)]
